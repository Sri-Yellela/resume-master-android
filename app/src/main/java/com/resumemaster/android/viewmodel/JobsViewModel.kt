...
class JobsViewModel:ViewModel(){ private val repository=JobRepository(); val jobs=repository.jobs; private val _currentIndex=MutableStateFlow(0); val currentIndex:StateFlow<Int> = _currentIndex; private val _lastAction=MutableStateFlow<SwipeAction?>(null); val lastAction:StateFlow<SwipeAction?> = _lastAction; val queue=mutableListOf<Job>(); val starred=mutableListOf<Job>(); fun onSwipe(action:SwipeAction,job:Job){ when(action){ SwipeAction.Queue, SwipeAction.Apply -> if(queue.none{it.id==job.id}) queue.add(job); SwipeAction.Star -> if(starred.none{it.id==job.id}) starred.add(job); SwipeAction.Dislike, SwipeAction.Skip -> Unit }; _lastAction.value=action; repository.rotate(job); _currentIndex.value=0 }; fun onButtonAction(action:SwipeAction){ jobs.value.firstOrNull()?.let{onSwipe(action,it)} } }
