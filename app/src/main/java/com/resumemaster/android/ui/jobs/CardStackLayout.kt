...
private fun resolve(offset:Offset):SwipeAction?{ val ax=abs(offset.x); if(ax<SwipeThresholds.SOFT_OFFSET)return null; return if(offset.x<0) SwipeAction.Skip else if(offset.y < -60f) SwipeAction.Star else if(ax>SwipeThresholds.HARD_OFFSET) SwipeAction.QueuePriority else SwipeAction.Queue }
